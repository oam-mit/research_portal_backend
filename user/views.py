...
def login(request):
    ...
    if request.method=='POST':
        ...
        if user is not None:
            ...
            context['form']=form
            messages.error(request,'Please enter valid Email Id and password. Kindly note that both email id and password are case-sensitive')
    ...
    if request.method=='POST':
        ...
        if user_form.is_valid() and student_form.is_valid():
            ...
            try:
                ...
                messages.success(request,'Account Created successfully. Please check your inbox for the confirmation email. Failure to confirm your email id would result in failure to log in to your account')
                return redirect(reverse('user:login'))
            except:
                transaction.set_rollback(True)
                context['user_form']=user_form
                context['student_form']=student_form
                messages.error(request,'Internal Error occurred. Please try after sometime',extra_tags='danger')
