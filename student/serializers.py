...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def get_applied(self, obj):
        applications = Application.objects.filter(
            project=obj, student=self.context['user'].student)
...
class AppliedProjectSerializer(serializers.ModelSerializer):
    department_slug = serializers.CharField(
        source='faculty.user.department.slug',)
    faculty = UserSerializer(many=False, read_only=True, source='faculty.user')
...
class ApplicationSerializer(serializers.ModelSerializer):
    ...
    faculty = UserSerializer(many=False, read_only=True,
                             source='project.faculty.user')
    department_slug = serializers.CharField(
        source='project.faculty.user.department.slug')

    class Meta:
        model = Application
        fields = ['uuid_field', 'title', 'start_date',
                  'end_date', 'faculty', 'department_slug', 'status']
